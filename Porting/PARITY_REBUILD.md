...
`static_type_bindings.json` separately records 30 source-reviewed C# type
associations: seven individual entry/immigration mappings and 23 compact
room/maintenance mappings. Each identifies exact Java type declarations and
states known gaps at type or group level; `check_type_index.py` checks those
names against the two syntax indexes. These associations do not require
executable fixtures and do not count as completed parity. Group-level
`SourceMapped` confirms origin, not one-to-one implementation or behavior.
...
The next static gate is to review more C# type bindings and the corresponding Java
