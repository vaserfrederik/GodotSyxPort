...
`ImmigrationRuntime` is the fifth reviewed partial binding, to
`settlement/entry/Immigration.java`. The supplied Java source uses happiness
threshold 0.9 and applies the 0–2 clamp only when expected population is
nonzero. C# previously used 0.5 and clamped the zero-population case; both
are corrected. `ReferenceImmigrationFormula.java` extracts only the source's
numeric branches with explicit inputs, and CI compares boundary cases to the
production C# `WantedUltimately` method. The full Java game class cannot be
instantiated in this small harness; `Update` speed, incoming population,
auto-admission and persistence still differ or lack verification. The binding
stays `Porting`, never full parity.
...
`csharp_call_graph.json` is checked against both indexes and the five reviewed
