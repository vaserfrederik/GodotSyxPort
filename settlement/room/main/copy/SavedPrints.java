...
public final class SavedPrints {
	...
	private static String fileName = "SavedPrints71";
	...
	SavedPrints(ROOMS rooms){
		...
		try {
			Json[] jsons = new Json(PATHS.local().PROFILE.gets(fileName)).jsons("BLUEPRINTS");
			for (Json j : jsons) {
	...
	public void save() {
		...
		try {
			...
			json.save(PATHS.local().PROFILE.get(fileName));
